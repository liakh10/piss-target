...
const BOOT_LINES = ["unzipping fly", "shaking off nerves", "pressurizing tank", "aiming true"];
...
export default function Enter() {
  ...
  const wake = () => {
    ...
    getSfx().start();
    setLeaving(true);
    window.dispatchEvent(new Event("piss:awake"));
  ...
  return (
    <div className={`enter ${leaving ? "enter-leaving" : ""}`}>
      {phase === "boot" && (
        <div className="boot">
          <p className="boot-title">PISS TARGET</p>
      ...
      {phase === "wake" && (
        <button className="wake" onClick={wake} aria-label="Tap to launch">
          <span className="wake-img"><Image src="/art/pisser.png" alt="" width={220} height={220} priority /></span>
          <span className="wake-cta">tap to launch</span>
          <span className="wake-sub">sound on · aim and fire, soak every target</span>
        </button>
      )}
    </div>
  );
