// Minimal WebAudio SFX — squirty, goofy, bathroom-arcade. Lazy after a gesture.
export class Sfx {
  private ctx: AudioContext | null = null;
  enabled = true;
  constructor() { try { this.enabled = localStorage.getItem("piss_muted") !== "1"; } catch { /* */ } }
  ...
  setEnabled(b: boolean) { this.enabled = b; try { localStorage.setItem("piss_muted", b ? "0" : "1"); } catch { /* */ } }

  squirt() { this.noise(0.14, 3200, 0.05, true); this.blip(520, 0.1, "sine", 0.03, 720); }
  hit() { this.blip(880, 0.09, "square", 0.05, 1320); this.blip(1180, 0.08, "triangle", 0.04, 1600); }
  miss() { this.noise(0.22, 700, 0.06); this.blip(160, 0.18, "sawtooth", 0.05, 70); }
  over() { this.blip(440, 0.16, "square", 0.05, 220); }
  start() { [392, 494, 587, 784].forEach((f, i) => setTimeout(() => this.blip(f, 0.12, "square", 0.05, f * 1.25), i * 70)); }
